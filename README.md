...
### Installer beta (Windows)

A Python-free Windows installer is now generated by GitHub Actions. It is not
yet a signed public release, so download and testing steps differ from the
eventual GitHub Releases experience.

**Japanese step-by-step instructions:** [INSTALL_BETA.md](INSTALL_BETA.md)

- Windows x64: beta installer available from the workflow artifacts
- macOS: not available yet; platform support is still being implemented
- Linux: source install below remains the supported method for now
...
> The implementation plan and release milestones are in
