...
- [x] **Step 5-2b**: Windows 実機での動作確認 実施（Windows 11 / RTX 4060 Laptop）
      → 実機で発見・修正したバグ2件:
      1. 初回生成の config.toml が cp932 で書かれ tomllib (UTF-8 必須) が
         クラッシュ → `write_text(..., encoding="utf-8")` に修正
      2. GPU 機で `device="auto"` が CUDA を選ぶが Windows に cuBLAS/cuDNN が
         無く、遅延 DLL ロードのため初回変換でハング（黄色のまま停止）
         → nvidia pip ホイールを Windows でも導入 + DLL フォルダを PATH に
         前置（`add_dll_directory` は ctranslate2 の LoadLibrary に効かない）
         + CUDA 読込直後にダミー音声で自己チェックし失敗時は CPU へ
         フォールバック。結果、**Windows GPU 推論が前倒しで対応完了**
...
### セットアップ
- [x] `run_windows.bat` のダブルクリックで venv 作成〜依存インストールが完走する
- [x] 2回目以降の起動はインストールをスキップして即起動する
- [x] コンソールウィンドウが表示されずに起動する（pythonw）
- [x] `%APPDATA%\voice-term\config.toml` が自動生成され、`key = "ctrl+alt"` になっている
      （※検証機ではユーザー希望で `cmd+alt` = Win+Alt に変更して運用）

### 基本動作
- [x] トレイに灰色ドット（読込中）→ 緑ドット（待機中）と遷移する
- [x] ホットキー長押しで赤ドット＋波紋アニメーションになる
- [x] 日本語を話す → 離す → フォーカス中のアプリに日本語が貼り付けられる
...
### エッジケース
- [x] 2重起動すると2つ目が「すでに起動しています」で終了する
- [ ] 0.3 秒未満の短い録音は無視される
- [ ] Ctrl+Alt を含む他アプリのショートカットと干渉しないか確認
  （干渉する場合は config で `f9` などに変更して回避できること）

### 既知の制限（確認のみ）
- ~~GPU (CUDA) は未対応~~ → 実機検証中に対応完了。nvidia pip ホイール
  （cuBLAS/cuDNN）を Windows でも導入し `auto` で GPU が使われる。
  GPU が動かない環境では起動時セルフチェックで CPU/int8 に自動フォールバック。
- トースト通知なし（トレイ表示のみ）。
